...
public class Game04_Boss : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < attackSkill.Length; i++)
        {
            ...
            attackSkill[i].attackValue = 60;
        }

        attackSkill[1].minAttackDistance = 1.8f;
        attackSkill[1].maxAttackDistance = 2.6f;
    ...
    void Update()
    {
        ...
        switch (monster.statue)
        {
            ...
            case en_MonsterSta.Run:
                ...
                {
                    moveDragTime = 0;
                    ChangeStatue(en_MonsterSta.Roll);
                    // Jump();
                    break;
                }
                // 没有固定点：            
                ...
                else if (runStatue == 1)
                {
                    if (runIndex == 0)
                    {
                        ...
                        //cube.transform.position = currMoveTargetPos;
                        //修改范围值，以及增加判断  
                        if (Vector3.Distance(transform.position, currMoveTargetPos) < 3.5f)
                        {
                            nextIndex = 1;
                ...
                {
                    targetPos = playersObj.transform.position + playersObj.transform.forward * 2.5f;
                    ...
                    //if (distance < monster.maxAttackDistance)
                    //    runStatue = 3;
                }
                else if (runStatue == 3)
                {
                    ...
                    {
                        //if ((animatorInfo.normalizedTime > 0.8f) && (animatorInfo.IsName("Roll")))//normalizedTime: 范围0 -- 1,  0是动作开始，1是动作结束
                        //{
                        //monster.animator.Play("Run");
                        if (attackSkillId == 0)
                        {
                        ...
                        else
                        {
                            ...
                            monster.animator.Play("KnifeRun");
            ...
            case en_MonsterSta.Roll:
                ...
                if ((animatorInfo.normalizedTime > 0.7f) && (animatorInfo.IsName("Roll")))
                {
                    ...
                    monster.tag = "Monster";
                    float _distance = Vector3.Distance(transform.position, playersObj.transform.position + playersObj.transform.forward * 2.5f);
                    if (nextIndex == 0 || _distance < 2.5f)
                    {
            ...
            case en_MonsterSta.Attack:
                ...
                if (attackSkill[attackSkillId].IsPlayEnd())
                {
                    ...
                    //animatorInfo = monster.animator.GetCurrentAnimatorStateInfo(0);
                    //if (!(animatorInfo.IsName("Roll")))
                    //    monster.animator.Play("Roll");
                    #endregion
            ...
            case en_MonsterSta.RunOut:
                ...
                //cube.transform.parent = null;
                //cube.transform.position = currMoveTargetPos;
                if (Vector3.Distance(transform.position, monster.mainCar_Obj.transform.position) > 2.5f && Vector3.Distance(transform.position, monster.mainCar_Obj.transform.position) < 4.5f)
                {
                    nextIndex = 1;
                    transform.LookAt(currMoveTargetPos);
                    ChangeStatue(en_MonsterSta.Roll);
                ...
                if (Vector3.Distance(transform.position, currMoveTargetPos) < 0.5f)
                {
                    ChangeStatue(en_MonsterSta.Run);
                    break;
                }
            ...
            case en_MonsterSta.Dmage:
                ...
                monster.animator.Play("Dmage");
                animatorInfo = monster.animator.GetCurrentAnimatorStateInfo(0);

                if ((animatorInfo.normalizedTime > 1f) && (animatorInfo.IsName("Dmage")))
                {
                    nextIndex = 1;
                    ChangeStatue(en_MonsterSta.Roll);
                    //Jump();
