        ...
        switch (monster.statue)
        {
            ...
            case en_MonsterSta.Run:
                ...
                else if (runStatue == 1)
                {
                    ...
                    {
                        RunCheck(currMoveTargetPos);
                        if (Vector3.Distance(transform.position, currMoveTargetPos) < 2.5f)
                        {
                            nextIndex = 1;
                ...
                else if (runStatue == 2)
                {
                    ...
                    else if (distance < 6.5f && distance > 4.5f)
                    {
            ...
            case en_MonsterSta.Dmage:
                ...
                animatorInfo = monster.animator.GetCurrentAnimatorStateInfo(0);
                //float _rotateY = transform.eulerAngles.y;
                //int tempRo = Random.Range(0, 2);
                //if (tempRo == 0)
                //    _rotateY = _rotateY + 10;
                //else
                //    _rotateY = _rotateY - 10;
                if ((animatorInfo.normalizedTime > 1f) && (animatorInfo.IsName("Dmage")))
                {
                    nextIndex = 1;
                    //  transform.eulerAngles = new Vector3(transform.eulerAngles.x, _rotateY, transform.eulerAngles.z);
                    ChangeStatue(en_MonsterSta.Roll);
                    break;
