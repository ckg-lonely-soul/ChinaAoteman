...
public class Game00_Boss : MonoBehaviour
{
    ...
    void ChangeStatue(en_MonsterSta sta)
    {
        ...
        switch (sta)
        {
            ...
            case en_MonsterSta.Jump:
                ...
                targetPos = gameMain.navMeshAgent_MainCar.transform.position + playersObj.transform.forward * Random.Range(4.5f, 5.7f) + playersObj.transform.right * Random.Range(-1.5f, 1.5f);// -8  8
    ...
    void UpdateNewTargetPos(bool drag)
    {
        ...
        if (drag)
        {
            offsetDistance = Random.Range(-4.5f, 4.5f);
        }
